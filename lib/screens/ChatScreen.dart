...
//custom
  void updateServerTime() async {
    print('call updateServerTime function');
    Map<String, String> serverTime = ServerValue.timestamp;
    await FirebaseDatabase.instance.ref().update({"currenttime": serverTime});
  }
  ...
  getCurrentTime() async {
    ...
    await FirebaseDatabase.instance.ref("currenttime").get().then((value) {
      ...
      print(
          'parse current time: ${Timestamp.fromMicrosecondsSinceEpoch(int.parse(currentTime!)).toDate()}');
    });
  }

  String getSendMessageTime() {
    print('call getSendMessageTime function');
    print('current time: $currentTime');
    print(
        Timestamp.fromMicrosecondsSinceEpoch(int.parse(currentTime!)).toDate());
    Duration duration =
        //DateTime.now().difference(DateTime.parse(currentTime!));

        DateTime.now().difference(
            Timestamp.fromMicrosecondsSinceEpoch(int.parse(currentTime!))
                .toDate());
    print("duration: $duration");
    return Timestamp.fromMicrosecondsSinceEpoch(int.parse(currentTime!))
        .toDate()
        .add(duration)
        .toUtc()
        .toString();
  ...
  loadDoctorProfile() async {
    ...
    await FirebaseDatabase.instance
      ...
      if (value.snapshot.value != null) {
        Map<dynamic, dynamic>.from(value.snapshot.value as dynamic)
            .forEach((key, values) {
          setState(() {
            tokensList.add(value.snapshot.child(key).value.toString());
          });
        });
      }
    });
    ...
    await FirebaseDatabase.instance
      ...
      if (value.snapshot.value != null) {
        if (mounted) {
          setState(() {
            requestStatus = (value.snapshot.child('status').value ?? 1) as int?;
          });
        }
      } else {
        setState(() {
  ...
  uploadDataWithBackgroundService(String taskId, result) async {
    ...
    dbRef.once().then((value) async {
      ...
      timeNow = value.snapshot.value.toString();
          ...
          dbRef2.set({
            ...
            "type":
                jsonDecode(result.response)['data'].toString().contains(".jpg")
                    ? 1
                    : 2,
            "messageCount": value.snapshot.value == null
                ? 1
                : (value.snapshot.child('messageCount').value! as int) + 1,
            "status": 0,
            "channelId": channelId
          });
        });
...
//get doctor presence
      ...
      setState(() {
        if (event.snapshot.value != null) {
          if (event.snapshot.child('presence').value != null) {
            if (event.snapshot.child('presence').value == true) {
              userStatus = "Online";
            } else {
              print(
  ...
  void sendMessage(int type) async {
    ...
    if (isFirstMessage) {
      ...
      await dbRef2.once().then((value) {
        dbRef2.set({
          ...
          "messageCount": value.snapshot.value == null
              ? 1
              : value.snapshot.child('messageCount').value == null
                  ? 1
                  : (value.snapshot.child('messageCount').value as int) + 1,
          "status": 0,
          "channelId": channelId
    ...
    if (messageCount >= 0) {
      setState(() {
        globalMessage =
            globalMessage!.isEmpty ? msg : globalMessage! + "`" + msg;
      });
    }
  ...
  void sendTask(int type, String taskId) async {
    ...
    DatabaseReference dbRef =
        FirebaseDatabase.instance.ref().child("currenttime").child("time");
    dbRef.once().then((value) async {
      print("got ref");
      timeNow = value.snapshot.value.toString();
      print("hhhh sendTask " + timeNow);
      CollectionReference collectionReference = FirebaseFirestore.instance
          .collection("Chats")
          .doc(channelId)
          .collection("All Chat");
  ...
  checkSeenStatus() async {
    checkSeenRefListner = FirebaseDatabase.instance
      ...
      if (event.snapshot.value == 0) {
        setState(() {
          messageCount = event.snapshot.value as int;
  ...
  Widget typeToWidget(int type, String message, String uid, String taskId,
    ...
    if (type == 1) {
      ...
      return InkWell(
        onTap: () async {
          if (isURL(message)) {
            if (await canLaunchUrlString(message)) {
